...
export default function NoteEditor() {
  ...
  async function remove() {
    ...
    await supabase.from("zhnote_notes").delete().eq("id", id); nav("/");
  }
  ...
  return (
    <div style={{ maxWidth: 760, margin: "0 auto" }}>
      ...
      <div className="card card-pad">
        <input className="input" style={{ fontSize: 20, fontWeight: 600, border: "none", padding: "6px 2px" }}
          placeholder="Tiêu đề" value={title} onChange={(e) => setTitle(e.target.value)} />
        <div className="divider" />
        <textarea className={"textarea " + (category === "tieng_trung" ? "zh" : "")}
          style={{ border: "none", minHeight: 320, padding: "2px", fontSize: 16 }}
      ...
      <div className="card card-pad stack" style={{ marginTop: 14 }}>
        ...
        <label className="row"><input type="checkbox" checked={pinned}
          onChange={(e) => setPinned(e.target.checked)} /><span>Ghim lên đầu</span></label>
      </div>
    </div>
  );
}
